        ...
        db.get('SELECT value FROM game_stats WHERE name = ?', ['total_in'], (err, totalInRow) => {
            db.get('SELECT value FROM game_stats WHERE name = ?', ['total_out'], (err, totalOutRow) => {
                db.get('SELECT balance FROM users WHERE unique_id = ?', [unique_id], (err, user) => {
                    ...
                    let win = current_rtp < target_rtp ? Math.random() < 0.5 : false;
                    let win_lines = [];
                    let tot_win = 0;
                    let pattern;
                    if(win){
                    ...
                    res.json({ res: true, win: win, pattern: pattern, win_lines: win_lines, money: new_balance, tot_win: tot_win, freespin: false, num_freespin: 0, bonus: false, bonus_prize: -1 });
                });
            });
        });
    });
...
app.get('/admin/dashboard', isAdmin, (req, res) => { res.sendFile(path.join(__dirname, 'admin', 'dashboard.html')); });
...
app.post('/admin/add-credit', isAdmin, (req, res) => {
    ...
    db.run('UPDATE users SET balance = balance + ? WHERE unique_id = ?', [amount, unique_id], (err) => {
        if (err) {
